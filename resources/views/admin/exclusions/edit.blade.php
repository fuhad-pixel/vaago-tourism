...
<div class="admin-page-panel">
    <div class="page-panel-header">
        <div class="panel-header-action-container">
            <h3><i class="fa-solid fa-pen-to-square"></i> Edit Exclusion</h3>
            <a href="{{ url('/admin/additional-exclusions') }}" class="btn-add-new">
                <i class="fa-solid fa-arrow-left-long"></i> Back to Exclusions
            </a>
        </div>
    </div>
    
    <div class="page-panel-body">
        <form action="{{ url('/admin/additional-exclusions/' . $exclusion->id) }}" method="POST" class="validate-form">
            @csrf
            @method('PUT')
            ...
            <div class="form-section-grid">
                ...
                <div>
                    <h4 class="form-section-title"><i class="fa-solid fa-circle-info"></i> Exclusion Details</h4>
                    
                    <div class="form-group">
                        <label for="name">Exclusion Name <span class="text-danger">*</span></label>
                        <input type="text" name="name" id="name" class="modern-input" placeholder="e.g. Flight Tickets" value="{{ old('name', $exclusion->name) }}" required>
                        @error('name')
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="icon" id="icon" class="modern-input" placeholder="e.g. fa-solid fa-plane-slash" value="{{ old('icon', $exclusion->icon) }}" required>
                        @error('icon')
                            <span class="text-danger" style="font-size: 0.85rem; margin-top: 4px; display: block;">{{ $message }}</span>
                        @enderror
                ...
                <div style="display: flex; flex-direction: column; justify-content: center; align-items: center; border-left: 1px solid #F3F4F6; padding: 20px;">
                    ...
                    <div id="icon-preview-box" style="display: flex; justify-content: center; align-items: center; width: 120px; height: 120px; background: #FFF8F5; border: 2px dashed rgba(241, 93, 48, 0.2); border-radius: 20px; color: #f15d30; font-size: 3.5rem; transition: all 0.3s ease;">
                        <i class="{{ $exclusion->icon }}" id="icon-preview-el"></i>
                    </div>
                    <p style="margin-top: 15px; font-weight: 600; color: var(--text-secondary);" id="icon-preview-name">{{ $exclusion->icon }}</p>
                </div>
            </div>
            ...
            <div style="display: flex; justify-content: flex-end; gap: 12px;">
                <a href="{{ url('/admin/additional-exclusions') }}" class="modern-btn" style="background: #E5E7EB; color: var(--text-primary);">
                    Cancel
                </a>
                <button type="submit" class="modern-btn">
                    <i class="fa-solid fa-floppy-disk"></i> Update Exclusion
                </button>
            </div>
        </form>
    </div>
