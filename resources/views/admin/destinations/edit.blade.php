...
<div class="admin-page-panel">
    <div class="page-panel-header">
        <div class="panel-header-action-container">
            <h3><i class="fa-solid fa-pen-to-square"></i> {{ $isParent ? 'Edit Parent Destination' : 'Edit Destination' }}: {{ $destination->name }}</h3>
    ...
    <div class="page-panel-body">
        <form action="{{ $isParent ? url('/admin/parent-destinations/' . $destination->id) : url('/admin/destinations/' . $destination->id) }}" method="POST" enctype="multipart/form-data" class="validate-form">
            @csrf
            @method('PUT')
            ...
            <div class="form-section-grid">
                ...
                <div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="parent_destination_id" id="parent_destination_id" class="modern-input select2">
                            ...
                            @foreach($parentDestinations as $pDest)
                                <option value="{{ $pDest->id }}" {{ old('parent_destination_id', $destination->parent_destination_id) == $pDest->id ? 'selected' : '' }}>{{ $pDest->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="name" id="name" class="modern-input" placeholder="Enter destination name" value="{{ old('name', $destination->name) }}" required>
                        @error('name')
                            <span class="text-danger" style="font-size: 0.85rem; margin-top: 4px; display: block;">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="description" id="description" class="modern-input ckeditor-init" rows="10">{{ old('description', $destination->description) }}</textarea>
                        @error('description')
                            <span class="text-danger" style="font-size: 0.85rem; margin-top: 4px; display: block;">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="image-upload-wrapper {{ $destination->image ? 'has-image' : '' }}">
                            <input type="file" name="image" class="image-upload-input" accept="image/*">
                            <div class="upload-placeholder" style="{{ $destination->image ? 'display: none;' : '' }}">
                                <i class="fa-solid fa-cloud-arrow-up"></i>
                                <span>Upload New Image</span>
                            </div>
                            <img src="{{ $destination->image ? asset($destination->image) : '' }}" class="image-preview" alt="Destination Preview" style="border-radius: 8px; {{ $destination->image ? 'display: block;' : '' }}">
                            <div class="image-remove-btn" style="{{ $destination->image ? 'display: flex;' : '' }}"><i class="fa-solid fa-xmark"></i></div>
                        </div>
                        @error('image')
                            <span class="text-danger" style="font-size: 0.85rem; margin-top: 4px; display: block;">{{ $message }}</span>
                        @enderror
                        ...
                        @if($destination->image)
                            <p style="font-size: 0.8rem; color: var(--success); margin-top: 4px;"><i class="fa-solid fa-circle-check"></i> Current image uploaded.</p>
                        @endif
                    </div>
                </div>
            </div>
            ...
            <div style="display: flex; justify-content: flex-end; gap: 12px;">
                ...
                <button type="submit" class="modern-btn">
                    <i class="fa-solid fa-floppy-disk"></i> Update Destination
                </button>
            </div>
        </form>
    </div>
